...
module.exports = function drophistory(mod) {
	...
	config.whitelist.forEach(item => {
		itemNames.set(+item, mod.game.data.items.get(item).name);
	});
	...
	mod.command.add('dh', {
		...
		log(...args) {
			if (args.length === 0) {
				mod.command.message(
					'Usage: dh log [file|chat|party|self] to toggle modes',
				);
				return;
			}
			...
			args.foreach(arg => {
				arg = arg.toLowerCase();
				if (config.logMode[arg]) config.logMode[arg] = !config.logMode[arg];
			});

			// config.logToFile = args.includes('file')
			// 	? !config.logToFile
			// 	: config.logToFile;

			// config.logToChat = args.includes('chat')
			// 	? !config.logToChat
			// 	: config.logToChat;

			// config.partyLogging = args.includes('party')
			// 	? !config.partyLogging
			// 	: config.partyLogging;

			// config.selfLogging = args.includes('self')
			// 	? !config.selfLogging
			// 	: config.selfLogging;
			...
			if (config.logMode.party) addPartyMembers();
			if (!config.logMode.file) saveLog();
			if (config.logMode.self) AddToSetIfNew(mod.game.me.name);
		...
		whitelist: {
			add(item) {
				...
				item = item
					...
					.replace(/@.+$/, '')
					.trim();
				//Add item to whitelist and list of itemnames
				config.whitelist.push(parseInt(item));

				itemNames.set(+item, mod.game.data.items.get(item).name);
				mod.command.message(`added ${mod.game.data.items.get(item).name}`);
			},
			remove(item) {
				item = item
					.replace(/^<.*#+/, '')
	...
	function showCurrentLogState() {
		...
		// 	logFrom = 'nobody';
		// }
		if (config.logMode.self && config.logMode.party) {
			logFrom = 'self and party';
		} else if (config.logMode.self) {
			logFrom = 'self';
		} else if (config.logMode.party) {
			logFrom = 'party';
		} else {
			logFrom = 'nobody';
		}
		...
		// 	logTo = 'nowhere!';
		// }
		if (config.logMode.chat && config.logMode.file) {
			logTo = 'in chat and file';
		} else if (config.logMode.chat) {
			logTo = 'in chat';
		} else if (config.logMode.file) {
			logTo = 'in file';
		} else {
			logTo = 'nowhere!';
		}
		...
		mod.command.message(
			`Currently logging for: ${logFrom}, log is shown ${logTo}`,
		);
	}

	function normaliseItemToken(msg, e) {
		//Split up itemname into id and dbid (if existant) and streamline field names
		msg.tokens.ItemName = msg.tokens.ItemName.replace('@item:', '').split(
			'?dbid:',
		);
		if (msg.id == 'SMT_LOOTED_SPECIAL_ITEM') {
			msg.tokens.ItemName[1] = '' + e.uniqueId;
		}
		msg.tokens.UserName =
			'UserName' in msg.tokens
				? msg.tokens.UserName
				: msg.tokens.PartyPlayerName;
	}

	async function printChatLog(msg) {
		const rarity = mod.game.data.items.get(msg.tokens.itemName[0]).rareGrade;
		...
		let chatMsg = [
			...
			`${
				...
				(statItem ? statItem.statString : '')
			}`, //get localised String. if it's exo gear, append amount of stats
	...
	function handleSysMsg(e) {
		if (!config.enable || !(config.logMode.chat || config.logMode.file)) return;

		var msg = mod.parseSystemMessage(e.message ? e.message : e.sysmsg);
		if (
			msg.id == 'SMT_PARTY_LOOT_ITEM_PARTYPLAYER' ||
			msg.id == 'SMT_LOOTED_ITEM' ||
			msg.id == 'SMT_LOOTED_SPECIAL_ITEM'
		) {
			...
			if (!config.logMode.party && msg.tokens.UserName != mod.game.me.name)
				return;
			if (!config.logMode.self && msg.tokens.UserName == mod.game.me.name)
				return;

			if (config.logMode.chat) printChatLog(msg);
			if (config.logMode.file) updateFileLog(msg);
		}
	}
	...
	mod.hook('S_PARTY_MEMBER_LIST', 7, e => {
		...
		if (!config.logMode.party) return;
		addPartyMembers();
	});
